...
async function executeCommand(command: VscodeCommand) {
	try {
		...
		const { required, optional } = await command.steps.collectInputs(disableOptionalPrompts)
...
function registerCommands(): Disposable[] {
	return Commands.map(function (command) {
		return commands.registerCommand(command.key, async () => await executeCommand(command))
	})
}
 ...
 */
async function showCatchedError({ stderr, stdout, message }: CommandOutput & { message: string }) {
