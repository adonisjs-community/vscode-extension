...
const build = new CommandSteps(
	...
	[
		...
		{
			param: 'client',
			message: 'Select the package manager to decide which lock file to copy to the build folder',
			optional: true,
			type: ClientType,
		...
		{
			param: 'poll',
			message: 'Detect file changes by polling files instead of listening to filesystem events',
			optional: true,
			type: DataType.Boolean,
		},
	]
